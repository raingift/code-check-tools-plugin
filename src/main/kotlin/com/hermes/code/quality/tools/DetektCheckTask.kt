...
@CacheableTask
abstract class DetektCheckTask : DefaultTask() {
    @InputDirectory
    ...
    private fun executeDetekt(shouldCreateBaseLine: Boolean = false) {
        ...
        val reportValue = listOf(
            ...
            it.fileEnding + ":" + File(
                ...
                "${parameters.inputFile.asFile.get().name}-detekt-${it.fileNameSuffix}.${it.fileEnding}"
            ).absolutePath
        }
        ...
        execOperations.javaexec { task ->
            ...
            task.args(
                "--input", parameters.inputFile.asFile.get(),
